...
void Field::draw()
{
    ...
    if(X){
        gout << move_to(x+3,y+3) << color(255-r, 255-g,255-b) << line(sx-6,sy-6) << move_to(x+sx-3,y+3) << line(-sx+6,sy-6);
    }
    else if(O){
        gout << color(255-r, 255-g,255-b);
        for(int i=x; i<x+sx; i++){
            for(int j=y; j<y+sy; j++){
                if((pow(i-(x+sx/2),2)+pow(j-(y+sy/2),2))<pow((sx-6)/2,2)){
        ...
        for(int i=x; i<x+sx; i++){
            for(int j=y; j<y+sy; j++){
                if((pow(i-(x+sx/2),2)+pow(j-(y+sy/2),2))<pow((sx-8)/2,2)){
                    gout << move_to(i,j) << dot;
                }
            }
        }
